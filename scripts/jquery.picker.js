...
(function ($) {
    ...
    $.fn.picker = function (json, callback) {
        ...
        $.each(options, function (index, option) {
            ...
            $ele.find('.picker-scroller').append('<div class="option stroked">' + option + '</div>');
        });
        $ele.find('.clone-scroller').bind("scroll", function () {
            ...
            let pickerOptionTop = $(".picker-up").offset().top + 80;
            let pickerOptionBottom = $(".picker-down").offset().top;
            
            $(".picker-scroller .option").each(function(){
                if ($(this).offset().top < pickerOptionTop)  {
                    $(this).addClass('stroked', 2000);
                }
                else if ($(this).offset().top + 120 > pickerOptionBottom){
                    $(this).addClass('stroked', 2000);
                }
               else {
                $(this).removeClass('stroked', 2000);
               }
            })
    ...
    $scroller.css("-webkit-transform", "translateZ(-90px) rotateX(" + unit + "deg)");
        ...
        /*setting css*/
        if (typeof json.lineHeight != "undefined") {
            $ele.css("height", (lineHeight * 3) + "px");
            ...
            $ele.find(".picker-down").css("top", (lineHeight * 2) + "px");
        }
        // default selected
        ...
        $ele.find('.picker-scroller').find(".option").each(function (index, $option) {
            $option = $($option);
            $option.css("-webkit-transform", "rotateX(-" + (22.5 * index) + "deg) translateZ(300px)");
            if (index > 7) {
                $option.hide();
            }
        });
